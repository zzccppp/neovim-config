local list = require("plugin-config.mason.lsp.list")
...
local default_config = require("plugin-config.mason.lsp.default")
...
for _, value in pairs(servers) do
	local status, config = pcall(require, "plugin-config.mason.lsp.config." .. value)
	if not status then
		config = {}
	end
...
for _, ele in pairs(alones) do
	require("plugin-config.mason.lsp.config." .. ele)
end
